...
export default function App() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: (created) => {
      if (currentUserId !== null) {
        qc.setQueryData<Task[]>(QUERY_KEYS.userTasks(currentUserId), (prev) => [created, ...(prev ?? [])]);
      }
  ...
  const statusMut = useMutation({
    mutationFn: ({ taskId, status, data, nextUserId }: { taskId: number; status: number; data: Record<string, any>; nextUserId: number }) =>
  ...
  const busy = usersLoading || tasksLoading || createMut.isPending || statusMut.isPending || closeMut.isPending;
    ...
    <div className="container">
      ...
      <TaskDialog
        ...
        onSave={async (taskId, data, nextUserId) => {
          ...
          return await statusMut.mutateAsync({ taskId, status: t.status, data, nextUserId });
        }}
        onForward={async (taskId, data, nextUserId) => {
          ...
          return await statusMut.mutateAsync({ taskId, status: t.status + 1, data, nextUserId });
        }}
